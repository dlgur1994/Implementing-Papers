...
# Test Mode
    ...
    with torch.no_grad():
        ...
        for batch, data in enumerate(loader_test, 1):
            ...
            # intersection = label * output
            # union = label + output
            iou_score = np.sum(intersection) / np.sum(union)
            iou_scores.append(iou_score)
            print("TEST: BATCH %04d \ %04d | IoU %.4f"
                  % (batch, num_batch_test, iou_score))
    ...
    print("AVERAGE TEST: BATCH %04d \ %04d | mIoU %.4f"
          % (batch, num_batch_test, np.mean(iou_scores)))